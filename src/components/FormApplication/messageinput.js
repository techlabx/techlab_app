    ...
    renderButtons = event => {
        return (<Fragment>
            {this.props.options.map((option) => (
                <input value={option} type="button" key={option} className={styles.buttoninput} onClick={this.handleClickButton} onBlur={this.returnToDefault}/>
            ))}
        </Fragment>)
    }

    renderInput = () => {
        if (this.props.options ? this.props.options.length > 0 ? true : false : false) {
